...
[guyue55/MacRightClick](https://github.com/guyue55/MacRightClick) 精简，提供新建文件、拷贝路径、必要设置与诊断能力。
...
“拷贝路径”与“新建文件”平级：在空白处复制当前目录的本机绝对路径；在文件或文件夹上右键则复制该项目本身的路径。Finder 原生菜单多选时，每行一个路径。路径保留中文和空格，不添加 `file://` 前缀；云盘项目复制本机路径，不生成分享链接或下载内容。复制成功通知沿用“显示成功通知”开关。

对于 iCloud Drive、同步桌面、OneDrive 等会抑制第三方 FinderSync 菜单的
File Provider 目录，可在“Finder → 云盘兼容菜单”中启用 `⌘ + 右键`。该手势
弹出同时包含“新建文件”和“拷贝路径”的独立菜单，并复用全部格式、模板、命名与高亮逻辑。“拷贝路径”沿用该菜单的鼠标命中项目／空白处目录解析，与原生菜单调用同一执行器；普通右键
仍由 Finder 原样处理。
...
```bash
git tag -a v1.0.5 -m "1.0.5：新增拷贝路径，改用 macOS 原生通知，成功默认静默"
...
1. Finder 空白处及文件／文件夹上右键能看到平级的“新建文件”和“拷贝路径”。
