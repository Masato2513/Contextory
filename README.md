...
Contextory 是一个面向个人使用的 macOS Finder 右键文件工具，中文显示名为“右键助手”。名称由 Context 与 Factory 组合而来，表达“右键上下文的工具工厂”。当前分支基于
[guyue55/MacRightClick](https://github.com/guyue55/MacRightClick) 精简，提供新建文件、复制路径、必要设置与诊断能力。
...
- 创建成功后 Finder 保留新文件的高亮选中；成功通知默认关闭，可在“概览 → 通知”中开启“显示成功通知”；已有开关选择会保留。

“复制路径”与“新建文件”平级：在空白处复制当前目录的本机绝对路径；在文件或文件夹上右键则复制该项目本身的路径。Finder 原生菜单多选时，每行一个路径。路径保留中文和空格，不添加 `file://` 前缀；云盘项目复制本机路径，不生成分享链接或下载内容。复制成功通知沿用“显示成功通知”开关。

对于 iCloud Drive、同步桌面、OneDrive 等会抑制第三方 FinderSync 菜单的
File Provider 目录，可在“Finder → 云盘兼容菜单”中启用 `⌘ + 右键`。该手势
弹出同时包含“新建文件”和“复制路径”的独立菜单，并复用全部格式、模板、命名与高亮逻辑。“复制路径”沿用该菜单的鼠标命中项目／空白处目录解析，与原生菜单调用同一执行器；普通右键
仍由 Finder 原样处理。
...
当前实现不会在关闭动画后销毁设置视图或主动调用 malloc 回收。设置页共用一个串行检测入口，避免重复创建检测任务；动作队列合并唤醒信号，每批最多领取 32 个事件，当前动作完成后再继续。Finder 扩展只保留菜单描述与动作投递代码。内存变化应在相同设置、相同窗口状态下实测，不能用可执行文件大小推算。

## 设置功能

- 概览：Finder Extension 状态、登录时启动、后台静默启动、成功通知开关、通知授权状态与系统通知设置入口。
...
1.0.5 使用 macOS 原生通知替代自绘悬浮提示，通知的外观、位置、展示时长和关闭方式由系统管理。

- 成功默认静默；需要时在“概览 → 通知”开启“显示成功通知”。
- 失败发送系统通知，不受成功通知开关影响；成功、失败两类各只保留最新一条，均不播放声音。
- 首次失败、首次主动开启成功通知，或点击“允许通知…”时请求系统授权；后台启动和登录启动不主动请求。
- 点击通知会打开设置。最近一次失败也会显示在设置页顶部，菜单栏图标旁出现 `!`，可通过“查看最近失败…”查看并在设置页清除。
- 拒绝授权、关闭横幅或启用专注模式时，系统可能不显示横幅；当前运行期间仍可从菜单栏和设置页查看最近一次失败。
- 如需调整展示方式，点击“通知设置…”，在系统设置中找到“右键助手”。通知发送失败或权限被关闭时，授权状态区域会给出相应说明。

## 离线与隐私

运行时只进行本地文件操作、用户主动触发的剪贴板写入、系统日志记录、本地系统通知，以及 Finder Extension 与宿主 App 之间的本地动作队列通信。操作通知由本机提交，不使用远程推送服务。启用云盘兼容菜单后，会使用本机辅助功能 API 识别 Finder 点击位置，并通过本机 Apple Events 读取当前 Finder 目录；这些操作不经过网络。源码已移除更新 UI、更新检查、外部工具下载和无效的云盘路径注册逻辑。
...
仓库内置 `.github/workflows/release.yml`。推送与 `VERSION` 一致的版本标签时，GitHub Actions 会在 Apple Silicon macOS runner 上自动运行测试、构建并校验产物，然后创建 GitHub Release，使用附注标签中的说明作为发布说明，并上传 DMG、ZIP 与 SHA-256 校验文件。

发布 1.0.5（先提交并推送对应代码，再创建附注标签）：

```bash
git tag -a v1.0.5 -m "1.0.5：新增复制路径，改用 macOS 原生通知，成功默认静默"
...
1. Finder 空白处及文件／文件夹上右键能看到平级的“新建文件”和“复制路径”。
2. 七种常用格式均可创建并按规则处理同名文件。
3. “其他…”能创建带自定义后缀的普通空文件。
4. 创建后文件保持高亮；关闭“显示成功通知”后不发送成功通知，开启并授权后使用系统通知。
...
1. 普通目录空白处点击“复制路径”，粘贴结果为当前目录；文件和文件夹上右键时结果为该项目本身。
2. Finder 原生菜单中多选项目后复制，每行一个项目路径，中文和空格保持原样。
3. 在 iCloud Drive、同步桌面等目录通过 `⌘ + 右键`复制，分别验证空白处和鼠标下的文件／文件夹；确认新建文件仍按原有逻辑执行。
4. 深浅色模式下，“复制路径”图标与现有菜单的尺寸、颜色一致。

通知功能的实机验收：

1. 在“概览 → 通知”点击“允许通知…”，确认系统授权流程和状态显示正常。
2. 触发一次操作失败，确认系统通知、菜单栏 `!` 和设置页失败说明一致；点击通知能打开设置。
3. 在系统设置中关闭通知后再次触发失败，确认菜单栏和设置页仍能查看错误。
4. 开启成功通知后连续创建文件，确认通知中心只保留最新成功通知，关闭开关后不再发送。
5. 清除设置页的失败提示，确认菜单栏标记和对应失败通知随之清除。
