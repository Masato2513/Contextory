...
///
/// - Subsystem 固定为 `io.github.masato2513.Contextory`
/// - 通过 `log show --predicate 'subsystem == "io.github.masato2513.Contextory"'` 可一次性看到所有 category
/// - debug 级在生产环境零开销（os.Logger 默认不持久化 .debug）
/// - 对外仅暴露 info / debug / error 三档，避免调用方滥用 default 等扰乱级别
public enum AppLog {
    public static let subsystem = "io.github.masato2513.Contextory"

    private static let registryQueue = DispatchQueue(label: "io.github.masato2513.Contextory.AppLog.registry")
