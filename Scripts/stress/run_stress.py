#!/usr/bin/env python3
"""
Real-machine stress harness for Contextory.
...
Run:
    python3 Scripts/stress/run_stress.py --burst 200 --malformed 5 --concurrency 8
"""
...
EXT_BUNDLE_ID = "io.github.masato2513.Contextory.Extension"
...
# 精简版只保留新建文件动作。压测使用独立临时目录创建空 TXT，结束后清理。
SAFE_ACTIONS = [
...
def host_pid() -> int | None:
    out = subprocess.run(
        ["pgrep", "-f", "右键助手.app/Contents/MacOS/Contextory"],
        capture_output=True,
        text=True,
    )
...
def main(argv: list[str]) -> int:
    ...
    p.add_argument(
        "--target",
        help="用于创建压测文件的目录；默认自动创建并清理临时目录",
    )
    args = p.parse_args(argv)

    owns_target = args.target is None
    target = pathlib.Path(args.target).resolve() if args.target else pathlib.Path(
        tempfile.mkdtemp(prefix="ContextoryStress-")
    )
    ...
    results["burst"] = run_burst(args.burst, args.concurrency, str(target))
