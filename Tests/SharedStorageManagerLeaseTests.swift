...
final class SharedStorageManagerLeaseTests: XCTestCase {
    ...
    func testInjectedConfigurationBypassesAppGroupDefaults() throws {
        ...
        let suiteName = "io.github.masato2513.ContextoryTests.\(UUID().uuidString)"
    ...
    func testMASConfigurationMirrorsCanonicalConfigToInjectedDefaults() throws {
        ...
        let suiteName = "io.github.masato2513.ContextoryTests.mas.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        addTeardownBlock { [suiteName] in
            UserDefaults(suiteName: suiteName)?.removePersistentDomain(forName: suiteName)
        }
    ...
    func testConfigurationWriteFailureIsReportedBeforeDefaultsAreMirrored() throws {
        ...
        let suiteName = "io.github.masato2513.ContextoryTests.write-failure.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
    ...
    func testRejectedSubmissionAcknowledgesLeaseImmediately() throws {
        ...
        try Data().write(to: target)
        let action = ControllableAsyncAction(acceptsSubmission: false)
        let dispatcher = ActionDispatcher()
        dispatcher.register(action: action)
        _ = try manager.enqueueAction(actionId: action.actionId, paths: [target.path])
        let lease = try XCTUnwrap(manager.consumePendingActionLeases().first)
...
private final class ControllableAsyncAction: MenuAction, @unchecked Sendable {
    ...
    private let acceptsSubmission: Bool

    private let lock = NSLock()
    private var pendingCompletion: (@Sendable (ActionCompletionStatus) -> Void)?

    init(acceptsSubmission: Bool = true) {
        self.acceptsSubmission = acceptsSubmission
    }
    ...
    func submit(
        ...
        guard acceptsSubmission else { return .rejected }
        lock.lock()
