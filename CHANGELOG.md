...
- 新增与“新建文件”平级的“拷贝路径”：空白处复制当前目录，文件或文件夹上复制项目本身的绝对路径；Finder 原生菜单多选时按行复制各项目路径。
- 普通 Finder 菜单与云盘 `⌘ + 右键`兼容菜单复用同一复制动作和已有目标解析，图标沿用当前 16 pt 单色样式；复制过程不读取文件内容。

### Changed

- 菜单、操作通知及当前说明统一使用“拷贝路径”文案，与 macOS 用语保持一致。
- “拷贝路径”改用带端点的曲线路径图标，普通 Finder 菜单与云盘兼容菜单同步更新，保留 16 pt 单色样式。
- 成功和失败提示改为 macOS 原生通知，移除自绘 HUD、窗口动画和关闭计时器；通知外观、位置和展示方式由系统管理。
- 成功通知默认关闭并保留已有开关偏好；失败发送系统通知，不播放声音，同类通知仅保留最新一条。
- 新建成功通知正文仅显示实际创建的文件名，包含同名文件自动追加的编号。
- 设置页增加通知授权状态和系统设置入口；首次失败或主动启用时请求授权，后台启动不打扰。
- 菜单栏和设置页保留最近一次失败，通知未授权或被专注模式抑制时仍可查看；点击通知打开设置。
